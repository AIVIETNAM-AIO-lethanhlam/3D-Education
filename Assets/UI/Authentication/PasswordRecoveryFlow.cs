...
public sealed class PasswordRecoveryFlow : IDisposable
{
    ...
    public void ApplyLanguage()
    {
        ...
        if (currentStep == Step.ResetPassword)
    ...
    private void LocalizeElementTree(VisualElement element)
    {
        ...
        if (element is Label label)
            label.text = TranslateKnownRecoveryText(label.text);

        if (element is Button button &&
            !string.IsNullOrWhiteSpace(button.text))
        {
            button.text = TranslateKnownRecoveryText(button.text);
        }
    ...
    private static string TranslateKnownRecoveryText(string text)
    {
        ...
        switch (text.Trim())
        {
            ...
            case "Đặt lại mật khẩu thành công":
                return vi ? "Đặt lại mật khẩu thành công" : "Password reset successfully";
    ...
    private void ConfigureRecoveryPlaceholders()
    {
        ...
        {
            newPasswordField.textEdition.placeholder = L("Enter new password", "Nhập mật khẩu mới");
            newPasswordField.textEdition.hidePlaceholderOnFocus = false;
        }

        if (confirmPasswordField != null)
        {
            confirmPasswordField.textEdition.placeholder = L("Re-enter new password", "Nhập lại mật khẩu mới");
    ...
    private void HandleBack()
    {
        switch (currentStep)
        {
            case Step.ForgotPassword:
                // Back from Forgot Password always restores the normal Login UI.
                ReturnToLogin();
                break;
            ...
            case Step.ResetPassword:
                ...
                ShowMessage(
                    forgotMessageLabel,
                    L("Request a new code to restart password recovery.", "Hãy yêu cầu mã mới để bắt đầu lại quá trình khôi phục mật khẩu."),
                    MessageType.Info);
                forgotEmailField?.Focus();
                break;
    ...
    private void SendRecoveryCode()
    {
        ...
        {
            ShowMessage(forgotMessageLabel, L("Please enter your email address.", "Vui lòng nhập địa chỉ email."), MessageType.Error);
        ...
        {
            ShowMessage(forgotMessageLabel, L("Enter a valid email address.", "Vui lòng nhập địa chỉ email hợp lệ."), MessageType.Error);
            SetInputState(forgotEmailContainer, true, false);
            forgotEmailField?.Focus();
            return;
        }
    ...
    private IEnumerator SendRecoveryCodeCoroutine(string email, bool isResend)
    {
        ...
        {
            SetButtonLoading(sendCodeButton, true, L("Sending code...", "Đang gửi mã..."));
            forgotEmailField?.SetEnabled(false);
        }

        // Step 1: verify that the syntactically-valid email really exists
        // in Supabase Auth before asking Supabase to send a recovery code.
        bool emailExists = false;
        bool emailCheckCompleted = false;
        string emailCheckError = null;

        yield return SupabaseAuthService.CheckEmailExists(
            email,
            exists =>
            {
                emailExists = exists;
        ...
        if (!emailCheckCompleted ||
            !string.IsNullOrWhiteSpace(emailCheckError))
        {
            isSendingCode = false;
            ...
            if (isResend)
            {
                ...
                ShowMessage(
                    verifyMessageLabel,
                    L("Unable to verify this email right now. Please try again.", "Hiện không thể xác minh email này. Vui lòng thử lại."),
                    MessageType.Error);
            }
            else
            {
                SetButtonLoading(
                    sendCodeButton,
                    false,
                    L("Send verification code", "Gửi mã xác minh"));
            ...
            Debug.LogError(
                "[PasswordRecovery] Email existence check failed: " +
                emailCheckError);
        ...
        if (!emailExists)
        {
            ...
            if (isResend)
            {
                ...
                ShowMessage(
                    verifyMessageLabel,
                    L("This email is no longer registered with an account.", "Email này không còn được đăng ký với tài khoản."),
                    MessageType.Error);
            }
            else
            {
                ...
                ShowMessage(
                    forgotMessageLabel,
                    L("No account was found with this email address.", "Không tìm thấy tài khoản với địa chỉ email này."),
                    MessageType.Error);
        ...
        // Step 2: only an existing Supabase Auth email reaches /recover.
        string error = null;
        bool success = false;
        ...
        {
            SetButtonLoading(sendCodeButton, false, L("Send verification code", "Gửi mã xác minh"));
            forgotEmailField?.SetEnabled(true);
        }
        ...
        ShowMessage(
            ...
            isResend
                ...
                : L("A verification code has been sent to this email.", "Mã xác minh đã được gửi đến email này."),
            MessageType.Success);
    ...
    private void VerifyCode()
    {
        ...
        {
            ShowMessage(verifyMessageLabel, L("Enter the 6-digit verification code.", "Vui lòng nhập mã xác minh gồm 6 chữ số."), MessageType.Error);
            return;
        }
    ...
    private IEnumerator VerifyCodeCoroutine(string code)
    {
        ...
        SetButtonLoading(verifyCodeButton, true, L("Verifying...", "Đang xác minh..."));
        SetOtpEnabled(false);
        ClearMessage(verifyMessageLabel);
            ...
            string.IsNullOrWhiteSpace(response.access_token))
        {
            ...
            if (failedVerifyAttempts >= MaxFailedVerifyAttempts)
            {
                ...
                ShowMessage(
                    verifyMessageLabel,
                    L("Too many attempts. Please wait before trying again.", "Bạn đã thử quá nhiều lần. Vui lòng chờ trước khi thử lại."),
                    MessageType.Error);
            }
            else
            {
    ...
    private IEnumerator ResetPasswordCoroutine(string newPassword)
    {
        ...
        SetButtonLoading(resetPasswordButton, true, L("Resetting password...", "Đang đặt lại mật khẩu..."));
        ...
        SetButtonLoading(resetPasswordButton, false, L("Reset password", "Đặt lại mật khẩu"));
    ...
    private void UpdatePasswordValidation()
    {
        ...
        if (passwordMatchLabel != null)
        {
            ...
            {
                passwordMatchLabel.text = L("✓ Passwords match", "✓ Mật khẩu khớp");
                passwordMatchLabel.AddToClassList("password-match--success");
                passwordMatchLabel.RemoveFromClassList("password-match--error");
            }
            else
            {
                passwordMatchLabel.text = L("ⓘ Passwords do not match.", "ⓘ Mật khẩu không khớp.");
    ...
    private void UpdateResendButton()
    {
        ...
        if (resendSecondsRemaining > 0)
        {
            resendCodeButton.text =
                AppLanguageManager.IsVietnamese
                    ? $"Gửi lại sau 00:{resendSecondsRemaining:00}"
                    : $"Resend in 00:{resendSecondsRemaining:00}";
            resendCodeButton.SetEnabled(false);
        }
        else
        {
            resendCodeButton.text =
                L("Resend code", "Gửi lại mã");
    ...
    private static string TranslateRecoverySendError(string error)
    {
        ...
        {
            return L("Please wait a moment before requesting another code.", "Vui lòng chờ một lúc trước khi yêu cầu mã khác.");
        }
        ...
        {
            return L("Unable to connect to the server. Please check your internet connection.", "Không thể kết nối đến máy chủ. Vui lòng kiểm tra kết nối Internet.");
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(error))
            return L("The verification code is incorrect. Please try again.", "Mã xác minh không chính xác. Vui lòng thử lại.");

        string lower = error.ToLowerInvariant();

        if (lower.Contains("expired") || lower.Contains("otp_expired"))
            return L("This code has expired. Request a new code to continue.", "Mã này đã hết hạn. Hãy yêu cầu mã mới để tiếp tục.");

        if (lower.Contains("rate limit") || lower.Contains("too many requests"))
            return L("Too many attempts. Please wait before trying again.", "Bạn đã thử quá nhiều lần. Vui lòng chờ trước khi thử lại.");
    ...
    private static string TranslateResetError(string error)
    {
        ...
        if (lower.Contains("expired") ||
            ...
            lower.Contains("jwt"))
        {
            return L("Your recovery session has expired. Please request a new verification code.", "Phiên khôi phục đã hết hạn. Vui lòng yêu cầu mã xác minh mới.");
        }

        if (lower.Contains("password"))
            return L("The new password does not meet the account security requirements.", "Mật khẩu mới không đáp ứng yêu cầu bảo mật của tài khoản.");
